import {
  setWindowNamespace,
  windowOnMessage,
  providerSendMessage,
} from "@/libs/messenger/window";
import { ProviderName, ProviderType } from "@/types/provider";
import RadiantProvider from "@/providers/radiant/inject";
import { InternalMethods, InjectedIDs } from "@/types/messenger";
setWindowNamespace();
(window as Window).samara = {
  providers: {},
  settings: {},
};
const script = document.getElementById(InjectedIDs.main) as HTMLScriptElement;
const scriptURL = new URL(script.src);
window.samara.settings = JSON.parse(scriptURL.searchParams.get("settings")!);

windowOnMessage(async (msg): Promise<void> => {
  window["samara"]["providers"][msg.provider].handleMessage(msg.message);
});
window.addEventListener("load", () => {
  providerSendMessage(
    ProviderName.samara,
    JSON.stringify({ method: InternalMethods.newWindowInit })
  );
});
window.addEventListener("beforeunload", () => {
  providerSendMessage(
    ProviderName.samara,
    JSON.stringify({ method: InternalMethods.newWindowUnload })
  );
});
console.info("hello from injected code");
RadiantProvider(window, {
  name: ProviderName.radiant,
  type: ProviderType.radiant,
  sendMessageHandler: providerSendMessage,
});
