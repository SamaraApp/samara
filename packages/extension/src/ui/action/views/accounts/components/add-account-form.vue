<template>
  <div class="add-account-form__container">
    ...
    <div class="add-account-form">
      ...
      <p>
        Name your account to something that makes sense to you. Examples: Main account, dapp
...
import { SamaraAccount, KeyRecordAdd, WalletType } from "@samaraapp/types";
import Keyring from "@/libs/keyring/public-keyring";

const isFocus = ref(false);
const accountName = ref("");
const newAccount = ref<SamaraAccount | null>(null);
...
const setNewAccountInfo = async () => {
  ...
  await sendToBackgroundFromAction({
    ...
    if (res.result) {
      newAccount.value = JSON.parse(res.result) as SamaraAccount;
    }
  });
};
onMounted(() => {
