<template>
  <div class="app" :class="{ locked: isLocked }">
    <div ref="appMenuRef" class="app__menu d-none">
    ...
    <div v-if="isOpenMore" ref="dropdown" class="app__menu-dropdown-wrapper">
      <div class="app__menu-dropdown">
        <a class="app__menu-dropdown-link" @click="lockAction">
          <hold-icon /> <span>Lock Samara</span>
        </a>
...
import { MessageMethod } from "@/providers/radiant/types";
...
import RVMAccountState from "@/providers/radiant/libs/accounts-state";
...
const setNetwork = async (network: BaseNetwork) => {
  ...
  const inactiveAccounts: any[] = []; /*await kr.getAccounts(
...
const onSelectedAddressChanged = async (newAccount: SamaraAccount) => {
  accountHeaderData.value.selectedAccount = newAccount;
  if (currentNetwork.value.provider === ProviderName.radiant) {
    const evmAccountState = new RVMAccountState();
