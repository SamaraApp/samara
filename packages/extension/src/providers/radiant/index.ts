...
import getRequestProvider, { RequestClass } from "@samaraapp/request";
import Networks from "./networks";
import { MiddlewareFunction, OnMessageResponse } from "@samaraapp/types";
  ...
  implements BackgroundProviderInterface
{
  ...
  constructor(
    toWindow: (message: string) => void,
    network: RvmNetwork = Networks.radiant
  ) {
    super();
    ...
    this.namespace = ProviderName.radiant;
  ...
  setRequestProvider(network: BaseNetwork): void {
    this.network = network as RvmNetwork;
