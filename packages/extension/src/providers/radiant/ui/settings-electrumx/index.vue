...
import { GasPriceTypes } from "../../../../providers/radiant/libs/transaction/types";
import { GasFeeType } from "../../../../providers/radiant/ui/types";
import { RvmNetwork } from "../../types/rvm-network";
import { Rip20Token } from "../../types/rip20-token";
import BigNumber from "bignumber.js";
import { defaultGasCostVals } from "../common/default-vals";
import Transaction from "@/providers/radiant/libs/transaction";
...
import { isValidAddress } from "@/libs/utils/address-utils";
import {
  addressToP2PKH,
...
const props = defineProps({
  network: {
    type: Object as PropType<RvmNetwork>,
    default: () => ({}),
  },
  accountInfo: {
...
const loadingAsset = new Rip20Token({
...
const accountAssets = ref<Rip20Token[]>([]);
const selectedAsset = ref<Rip20Token | Partial<Rip20Token>>(loadingAsset);
...
const calculatedFee = ref<GasFeeInfo>(defaultGasCostVals);
...
const TxInfo = computed<SendTransactionDataType>(() => {
  ...
  return {
    ...
    from: addressFrom.value as `{string}`,
    value: value, // Multiply to get the satoshis
    to: toAddress as `${string}`,
    data: data as `0x${string}`,
    utxos: selectedAsset.value.unspentCoins,
  };
});

const Tx = computed(() => {
  const constructTxResult = constructTransaction(
    TxInfo.value.utxos,
    addressFrom.value,
    TxInfo.value.to,
    TxInfo.value.value * 100000000
  );
  setTransactionFees(constructTxResult.fee);
  return constructTxResult.tx;
});
...
const setTransactionFees = (fee: number) => {
  const nativeVal = accountAssets.value[0].price || "0";
  const calcFee = {
    nativeValue: new BigNumber(fee / 100000000),
    fiatValue: new BigNumber(fee / 100000000)
      .times(nativeVal)
      .toString(),
    nativeSymbol: props.network.currencyName,
    fiatSymbol: "USD",
  };
  setCalculatedFee(calcFee);
  return calcFee;
};
...
const fetchAssets = () => {
  ...
  return props.network
    .getAllTokens(addressFrom.value, true)
    .then((allAssets: any) => {
      accountAssets.value = allAssets as Rip20Token[];
      selectedAsset.value = allAssets[0] as Rip20Token;
...
const isInputsValid = computed<boolean>(() => {
  if (!isValidAddress(addressTo.value)) return false;
  if (!isValidDecimals(sendAmount.value, selectedAsset.value.decimals!)) {
    console.log("!isValidDecimals");
    return false;
  }
...
watch([isInputsValid, amount, addressTo, selectedAsset], () => {
  if (isInputsValid.value) {
    // No need to set, since it's done in the coin selection
...
const assetMaxValue = computed(() => {
  // Simulate the transaction, to determine the maximum amount that could be sent
  let theoreticalMax = 0;
  TxInfo.value.utxos.map((utxo) => {
    theoreticalMax += utxo.value;
  });

  if (selectedAsset.value.contract === NATIVE_TOKEN_ADDRESS) {
    let fee = getTransactionFeeEstimation(
      TxInfo.value.utxos,
      addressFrom.value,
      TxInfo.value.to || "1MLj6VUvonNFyp7NWKwPRCBtdmFB27haDE", // Dummy value for purposes of fee estimation
      theoreticalMax
    );
    fee /= 100000000;
    return fromBase(
...
const selectToken = (token: Rip20Token) => {
  inputAmount("0");
...
const setCalculatedFee = (fee: GasFeeInfo) => {
