...
import { constructTransaction, getTransactionFeeEstimation } from "@/libs/radiant-helpers/radiant-core-helpers";
import { WSSElectrumClient } from "@/providers/radiant/libs/electrumx_wsclient";
import { getTransitionInfo } from "../../../../../../../../../sensilet/src/utils/apiUtils";
...
const sendAction = async () => {
  ...
  for (const utxo of txData.TransactionData.utxos) {
    totalUtxos += utxo.value;
  }
  const estimateTxResult = getTransactionFeeEstimation(
    txData.TransactionData.utxos,
    txData.fromAddress,
    txData.toAddress,
    txData.toToken.amount
  );
  console.log("estimateTxResult", estimateTxResult);
  ...
  TransactionSigner({
    ...
    .then(async (response) => {
      const client: WSSElectrumClient = await WSSElectrumClient.Connection(network.node);
      client.sendTransaction(response.result).then((hash: string) => {
