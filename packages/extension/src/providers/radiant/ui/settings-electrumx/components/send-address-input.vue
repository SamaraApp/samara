<template>
  <div class="send-address-input" :class="{ focus: isFocus }">
    <div class="send-address-input__avatar">
      <img v-if="isValidAddress(value)" :src="network.identicon(value)" alt="" />
    </div>
    <div class="send-address-input__address">
      <p v-if="!from">To:</p>
      <p v-else>From:</p>
      <input ref="addressInput" v-model="address" type="text" :disabled="disableDirectInput" placeholder="Radiant address"
        :style="{ color: !isValidAddress(value) ? 'red' : '#eee' }" @focus="changeFocus" @blur="changeFocus" />
    </div>
  </div>
</template>
