...
import HWwallet from "@samaraapp/hw-wallets";
import { HWwalletType } from "@samaraapp/types";
...
const TransactionSigner = (
  ...
  if (account.isHardware) {
    ...
    const msgHash = payload; // "0x00"; //bufferToHex(payload.getMessageToSign(true));
    return sendUsingInternalMessengers({
      method: InternalMethods.sign,
      params: [msgHash, account],
    }).then((signedRawtx) => {
      if (!signedRawtx) {
        return Promise.reject("unable to sign");
      } else {
        return signedRawtx;
      }
    });
