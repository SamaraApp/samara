...
export class Provider extends EventEmitter implements ProviderInterface {
  ...
  async request(request: RadiantRequest): Promise<RadiantResponse> {
  ...
  //deprecated
  send(method: string, params?: Array<unknown>): Promise<RadiantResponse> {
  ...
  sendAsync(
    ...
    const { method, params } = data as RadiantRequest;
...
const injectDocument = (
  document: SamaraWindow | Window,
  options: ProviderOptions
): void => {
  const provider = new Provider(options);
  const globalSettings: SettingsType = document.samara.settings;
  if (!globalSettings.rvm.inject.disabled)
  ...
  document["samara"]["providers"][options.name] = provider;
};
