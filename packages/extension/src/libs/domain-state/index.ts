...
class DomainState {
  ...
  async setUnspentCoins(address: string, coins: any[]): Promise<void> {
    const state = await this.getState();
    state.unspentCoins = state.unspentCoins || {};
    state.unspentCoins[address] = coins;
    await this.setState(state);
  }

  async getUnspentCoins(address: string): Promise<any[] | null> {
    const state = await this.getState();
    if (state.unspentCoins && state.unspentCoins[address]) return state.unspentCoins[address];
    return null;
  }
